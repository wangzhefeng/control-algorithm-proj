...
class PID:
    ...
    def __init__(self, P, I, D) -> None:
        ...
        self.last_time = self.current_time  # 上次算法更新时间
        # 重置
        self.clear()
    
    def set_target_point(self, setpoint):
    ...
    def update(self, feedback_value):
        # 时间差
        self.current_time = time.time()  # 当前时间
        delta_time = self.current_time - self.last_time  # 执行时间
        ...
        if delta_time >= self.sample_time:
            ...
            self.I_term = self.I_term + error * delta_time  # 积分项
...
def run_pid(P, I , D, END):
    # 实例化 PID 类
    pid = PID(P, I, D)
    # 设置 PID 参数
    pid.set_target_point(setpoint = 1.1)  # 设置目标值
...
# 测试代码 main 函数
def main():
    run_pid(P = 1.2, I = 1, D = 0.001, END = 20)
